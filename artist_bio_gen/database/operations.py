            ...
            for attempt in range(max_retries + 1):  # +1 for initial attempt
                try:
                    ...
                    if error_type in ["permanent", "systemic"]:
                        logger.warning(
                            f"Database error ({error_type}): {str(e)} - not retrying"
                        )
                        break
                    ...
                    logger.warning(
                        f"Database operation failed (attempt {attempt + 1}/{max_retries + 1}): {str(e)} - retrying in {delay:.1f}s"
                    )
                    time.sleep(delay)
...
def update_artist_bio(
    ...
    if connection is None:
        return DatabaseResult(
            success=False, rows_affected=0, error="Database connection is None"
        )

    # Validate UUID format
    if not validate_uuid(artist_id):
        return DatabaseResult(
            success=False, rows_affected=0, error=f"Invalid UUID format: {artist_id}"
        )
    ...
    try:
        ...
        if rows_affected > 0:
            logger.debug(
                f"[{worker_id}] Updated bio for artist {artist_id} in {table_name}"
            )
            status = "updated"
        else:
            logger.debug(
                f"[{worker_id}] No rows updated for artist {artist_id} in {table_name} (may not exist or already has bio)"
            )
            status = "skipped"

        return DatabaseResult(success=True, rows_affected=rows_affected, error=None)
        ...
        error_type = classify_database_error(e)
        logger.error(
            f"[{worker_id}] Database error ({error_type}) updating artist {artist_id}: {str(e)}"
        )
